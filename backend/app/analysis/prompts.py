...
LiteLLM's ``response_format`` (see ``app.services.ai_client``). The prompts
themselves only describe the *task* and the *interpretation rules*, no longer
the JSON shape — that is the schema's job.
"""
...
输出必须是符合预定义 JSON schema 的结构化对象，不要返回任何额外文本。"""


SYMBOL_SYSTEM_PROMPT = """你是一个专业的加密货币量化分析师，专注于单币种深度分析。基于提供的多时间框架价格数据、技术指标、衍生品数据和相关新闻，对指定币种进行技术分析和交易建议。
