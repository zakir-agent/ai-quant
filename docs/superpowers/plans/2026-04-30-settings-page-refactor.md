...
**Goal:** Split the monolithic settings page.tsx (~750 lines) into modular card components and improve layout with section grouping.

**Architecture:** Extract 8 card sections from `page.tsx` into individual component files under `components/settings/`. Create a shared utilities module for `StatusDot`, `healthColor`, and a new `SectionHeader`. Rewrite `page.tsx` as a lightweight container. Apply layout grouping with section headers and visual polish.

**Tech Stack:** React, Next.js 16, Tailwind CSS 4, TypeScript
...
**Files:**
- Create: `frontend/src/components/settings/shared.tsx`

- [ ] **Step 1: Create shared.tsx with StatusDot, healthColor, SectionHeader**
...
Run: `cd /Users/zakir/work/ai-quant/.claude/worktrees/frolicking-hugging-chipmunk/frontend && npx tsc --noEmit --pretty 2>&1 | head -20`
Expected: No errors related to `shared.tsx`

- [ ] **Step 3: Commit**

```bash
git add frontend/src/components/settings/shared.tsx
git commit -m "refactor(settings): add shared utilities module"
```

---

### Task 2: Extract AiModelCard and AiUsageCard

**Files:**
- Create: `frontend/src/components/settings/AiModelCard.tsx`
- Create: `frontend/src/components/settings/AiUsageCard.tsx`

- [ ] **Step 1: Create AiModelCard.tsx**
...
```bash
git add frontend/src/components/settings/AiModelCard.tsx frontend/src/components/settings/AiUsageCard.tsx
git commit -m "refactor(settings): extract AiModelCard and AiUsageCard"
```

---

### Task 3: Extract DataSourcesCard and CollectionScheduleCard

**Files:**
- Create: `frontend/src/components/settings/DataSourcesCard.tsx`
- Create: `frontend/src/components/settings/CollectionScheduleCard.tsx`

- [ ] **Step 1: Create DataSourcesCard.tsx**
...
export default function DataSourcesCard({
  ...
  const rows = dataSourceRows.map((row) => {
    if (row.collector === "coingecko" || row.collector === "dexscreener" ||
        row.collector === "defillama" || row.collector === "fear_greed" ||
        row.collector === "news" || row.collector === "newsapi") {
...
- [ ] **Step 2: Create CollectionScheduleCard.tsx**
...
```bash
git add frontend/src/components/settings/DataSourcesCard.tsx frontend/src/components/settings/CollectionScheduleCard.tsx
git commit -m "refactor(settings): extract DataSourcesCard and CollectionScheduleCard"
```

---

### Task 4: Extract AlertingCard

**Files:**
- Create: `frontend/src/components/settings/AlertingCard.tsx`

- [ ] **Step 1: Create AlertingCard.tsx**
...
```bash
git add frontend/src/components/settings/AlertingCard.tsx
git commit -m "refactor(settings): extract AlertingCard"
```

---

### Task 5: Extract DataStatisticsCard and SchedulerJobsCard

**Files:**
- Create: `frontend/src/components/settings/DataStatisticsCard.tsx`
- Create: `frontend/src/components/settings/SchedulerJobsCard.tsx`

- [ ] **Step 1: Create DataStatisticsCard.tsx**
...
- [ ] **Step 2: Create SchedulerJobsCard.tsx**
...
```bash
git add frontend/src/components/settings/DataStatisticsCard.tsx frontend/src/components/settings/SchedulerJobsCard.tsx
git commit -m "refactor(settings): extract DataStatisticsCard and SchedulerJobsCard"
```

---

### Task 6: Extract DataIntegrityCard (most complex)

**Files:**
- Create: `frontend/src/components/settings/DataIntegrityCard.tsx`

- [ ] **Step 1: Create DataIntegrityCard.tsx**
...
```bash
git add frontend/src/components/settings/DataIntegrityCard.tsx
git commit -m "refactor(settings): extract DataIntegrityCard"
```

---

### Task 7: Rewrite page.tsx as container + apply layout

**Files:**
- Modify: `frontend/src/app/settings/page.tsx` (complete rewrite)

- [ ] **Step 1: Rewrite page.tsx as lightweight container with section grouping**
...
Run: `cd /Users/zakir/work/ai-quant/.claude/worktrees/frolicking-hugging-chipmunk/frontend && npm run lint 2>&1`
Expected: PASS
...
git commit -m "refactor(settings): rewrite page.tsx as lightweight container with section grouping"
```
...
Add these keys inside the `"settings"` object (after `"title"`):
...
Run: `cd /Users/zakir/work/ai-quant/.claude/worktrees/frolicking-hugging-chipmunk/frontend && npm run lint 2>&1 && npx tsc --noEmit --pretty 2>&1 | head -20`
Expected: Both PASS
...
git commit -m "i18n: add settings section headers and health status labels"
```
...
Run: `cd /Users/zakir/work/ai-quant/.claude/worktrees/frolicking-hugging-chipmunk/frontend && npx tsc --noEmit --pretty 2>&1`
Expected: No errors

- [ ] **Step 3: Build check**

Run: `cd /Users/zakir/work/ai-quant/.claude/worktrees/frolicking-hugging-chipmunk/frontend && npm run build 2>&1 | tail -20`
Expected: Build succeeds

- [ ] **Step 4: Update changelog.md**

Add entry under "未发布" section:
```
- 重构设置页面：拆分为 8 个独立组件，优化布局分组（AI/数据/系统）
```
...
git commit -m "docs: update changelog for settings refactor"
```
