...
function TimelineNodeInner({ report, isSelected, selectionOrder, onClick }: TimelineNodeProps) {
  const t = useT();
  ...
  return (
    <>
      ...
      {hoverRect &&
        createPortal(
          <div
            className="pointer-events-none fixed z-[9999] rounded-lg border border-[var(--border-primary)] bg-[var(--bg-card)] px-2.5 py-1.5 text-[10px] whitespace-nowrap shadow-lg"
            style={{
          ...
          >
            ...
            <div className="mt-0.5 text-[var(--text-muted)]">{riskLabel(report.risk_level, t)}</div>
          </div>,
          document.body,
        )}
    </>
